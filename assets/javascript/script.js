...
function runSimulation() {
    ...
    const CHECKPOINT_YEAR = new Date(`01 Jan ${this_year}`).getTime() //Date.parse(`${this_year}-01-01`); // Converts datetime string to epoch time
    const CHECKPOINT_DATE = new Date(`${this_year}-${date.getMonth() + 1}-${date.getDate()}`) //Date.parse(`${this_year}-${date.getMonth() + 1}-${date.getDate()}`);
